...
async function initRealtime() {
    ...
    onSnapshot(collection(window.db, "quizzes"), async (snap) => {
        ...
        snap.forEach(d => {
            ...
            } else if (q.author === displayName || playedIds.includes(d.id)) {
                btn.innerText = "Completado / Tu Quiz 🚫"; btn.disabled = true; btn.style.background = "#ccc";
            } else {
                btn.innerText = "Jugar 🎮"; btn.onclick = () => startQuizSession({id: d.id, ...q});
            }
            if (currentUser === ADMIN_ID || q.author === displayName) {
                const bS = document.createElement('button'); bS.className="btn-small"; bS.innerText="⚙️ Ajustes";
                bS.style.marginTop="5px"; bS.onclick = () => openSettings({id: d.id, ...q});
                div.appendChild(bS);
            }
            div.appendChild(btn);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-next-q').onclick = () => {
        ...
        document.getElementById('questions-added-count').innerText = `Preguntas: ${tempQuestions.length}`;
        resetEditorInputs();
    };
